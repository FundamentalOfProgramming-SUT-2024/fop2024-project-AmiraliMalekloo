...
pos* make_map(pos* begin,Game * g,int map[Rows-bRow][Cols-bCol],char show[Rows-bRow][Cols-bCol],char color[Rows-bRow][Cols-bCol][10],object obs[Rows-bRow][Cols-bCol],int floor){
    ...
    R[0]=gen_room(g,2,10,0,1,4,3*(x%3),A==0);    R[1]=gen_room(g,2,50,0,1,4,3*(x%3),A==2);    R[2]=gen_room(g,2,100,0,1,3,0,A==4);
    R[5]=gen_room(g,22,10,1,1,1,3*(x%3),A==1);    R[4]=gen_room(g,22,50,1,1,2,3*(x%3),A==3);    R[3]=gen_room(g,22,100,1,1,3,0,A==5);
    ...
    t_finish=map[Finish->x][Finish->y];
    find_path(map,begin,Finish,begin->x,begin->y,&mark,&parent);
    add_path(*begin,*Finish,parent,map);
    map[Finish->x][Finish->y]=t_finish;
    ...
    find_path(map,Start,ending,Start->x,Start->y,&mark,&parent);
    add_path(*Start,*ending,parent,map);
...
int main(){
    ...
    for(int i=0;i<4;i++){
        g.start[i]=*a;
        a=make_map(a,&g,Map[i],Show[i],Color[i],MAP[i],i+1);
        g.end[i]=*a;
    }

    print_map(Map[3],Show[3],Color[3]);
    refresh();
    sleep(20);
    while(1){
        clear();
