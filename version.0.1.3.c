...
//int make_path(Game* g,char map[Rows-bRow][Cols-bCol], pos* start, pos* end){
 //   return 0;
//}
...
Room* gen_room(int ver_0,int hor_0,int down){
    ...
    R->door_1=(pos *)malloc(sizeof(pos));
    R->door_2=(pos *)malloc(sizeof(pos));
    R->door_3=(pos *)malloc(sizeof(pos));
...
void add_room(Room* R,char map[Rows-bRow][Cols-bCol]){
    for(int i=R->ver;i<=R->ver+R->length;i++){
        for(int j=R->hor;j<=R->hor+R->width;j++){
            ...
            if(R->door_1->x==j&&R->door_2->y==i)  map[i][j]='+';
            if(R->door_2 != NULL) {if(R->door_2->x==j&&R->door_2->y==i) map[i][j]='+';}
            if(R->door_3->x==j&&R->door_3->y==i) map[i][j]='+';
        }
    }
}
...
int make_map(Game * g,char map[Rows-bRow][Cols-bCol],char show[Rows-bRow][Cols-bCol]){
    for(int i=0;i<Rows-bRow;i++){
        for(int j=0;j<Cols-bCol;j++){
            map[i][j]='O';
