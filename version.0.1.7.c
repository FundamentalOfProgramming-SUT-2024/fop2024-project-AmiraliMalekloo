...
int make_map(Game * g,int map[Rows-bRow][Cols-bCol],char show[Rows-bRow][Cols-bCol],char color[Rows-bRow][Cols-bCol][10],object obs[Rows-bRow][Cols-bCol],int floor);
...
Room* gen_room(Game* g,int ver_0,int hor_0,int down,int theme,int floor,int has_lock);
...
Room* gen_room(Game* g,int ver_0,int hor_0,int down,int theme,int floor,int lock_hide){  // ver_o : ->  hor_0 : ^
    int hard=0;
    int hide[3];
    int lock[3];
    int p=1;
...
int make_map(Game * g,int map[Rows-bRow][Cols-bCol],char show[Rows-bRow][Cols-bCol],char color[Rows-bRow][Cols-bCol][10],object obs[Rows-bRow][Cols-bCol],int floor){
    ...
    R[0]=gen_room(g,2,10,0,1,4,13);    R[2]=gen_room(g,2,50,0,1,4,3*(x%9));    R[4]=gen_room(g,2,100,0,1,3,3*(x%9));
    R[1]=gen_room(g,22,10,1,1,1,x%9);    R[3]=gen_room(g,22,50,1,1,2,x%9);    R[5]=gen_room(g,22,100,1,1,3,0);
...
int main(){
    ...
    for(int i=0;i<4;i++){
        make_map(&g,Map[i],Show[i],Color[i],MAP[i],0);
    }
