...
int new_game(){return 1;}
int prevoius_game(){return 1;}
int leader_board(){return 1;}
int setting(){return 1;}
int profile(){return 1;}

int before_game(){
    user_menu();
    pos cursor; cursor.x=Rows/2; cursor.y=Cols/2;
    mvprintw(1, 1, "Before Gameplay");
    mvprintw(Rows/2-4,Cols/2-20,"Start a new game");
    mvprintw(Rows/2-2,Cols/2-20,"Continue last game");
    mvprintw(Rows/2,Cols/2-15,"Leader board");
    mvprintw(Rows/2+2,Cols/2-10,"Settings");
    mvprintw(Rows/2+4,Cols/2-15,"Profile menu");
    while(1){
        noecho();
        int command=getch();
        ...
        if(command=='\n'){
            if(cursor.x==Rows/2-4){new_game();}
            if(cursor.x==Rows/2-2){prevoius_game();}
            if(cursor.x==Rows/2){leader_board();}
            if(cursor.x==Rows/2+2){setting();}
            if(cursor.x==Rows/2+4){profile();}
        }
